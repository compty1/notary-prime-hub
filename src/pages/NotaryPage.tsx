            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="flex flex-col items-center gap-6 text-center md:flex-row md:text-left">
              ...
              <div className="flex-1">
                ...
                <div className="mt-4 flex flex-wrap items-center justify-center gap-2 md:justify-start">
                  ...
                  {page.is_featured && <Badge className="gap-1 bg-amber-500 text-primary-foreground"><Star className="h-3 w-3" /> Featured</Badge>}
                  <CommissionBadge expirationDate={creds.commission_expiration} />
                </div>
                {creds.commissioned_state?.toLowerCase().includes("ohio") && (
                  <div className="mt-2">
                    <Badge variant="outline" className="gap-1 text-xs border-green-600 text-green-700 dark:text-green-400"><Shield className="h-3 w-3" /> Ohio Compliant — ORC §147</Badge>
                  </div>
                )}
