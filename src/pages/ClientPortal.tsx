        ...
        <Tabs defaultValue="appointments" className="space-y-6" onValueChange={val => {
              ...
              return (
                <Card key={req.id} className="border-border/50">
                  <CardContent className="p-4">
                    ...
                    <div className="flex items-center gap-1 my-3">{apoSteps.map((s, i) => (<div key={s} className="flex items-center flex-1"><div className={`flex h-6 w-6 items-center justify-center rounded-full text-[10px] font-bold ${i <= currentIdx ? "bg-gradient-primary text-white" : "bg-muted text-muted-foreground"}`}>{i < currentIdx ? "✓" : i + 1}</div>{i < apoSteps.length - 1 && <div className={`flex-1 h-0.5 mx-1 ${i < currentIdx ? "bg-accent" : "bg-muted"}`} />}</div>))}</div>
                    ...
                    <Card key={p.id} className="border-border/50">
                      <CardContent className="flex items-center justify-between p-4">
                        ...
                        <div className="flex items-center gap-2">
                          {p.status === "pending" && <Button size="sm" className="text-xs bg-gradient-primary text-white hover:opacity-90" onClick={() => setPayingPaymentId(p.id)}><CreditCard className="mr-1 h-3 w-3" /> Pay Now</Button>}
          ...
          <TabsContent value="reminders" className="space-y-6">
            ...
            <Card className="border-border/50"><CardContent className="p-4 space-y-4">
              ...
              }} size="sm" className="bg-gradient-primary text-white hover:opacity-90">{savingReminder ? <Loader2 className="mr-1 h-4 w-4 animate-spin" /> : <Bell className="mr-1 h-4 w-4" />} Set Reminder</Button>
            </CardContent></Card>
            {reminders.length === 0 ? (
      ...
      <Dialog open={editProfileOpen} onOpenChange={setEditProfileOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          <DialogFooter className="flex-col gap-2 sm:flex-row">
            ...
            <Button onClick={saveProfile} disabled={savingProfile} className="bg-gradient-primary text-white hover:opacity-90">{savingProfile ? <Loader2 className="mr-1 h-4 w-4 animate-spin" /> : <Save className="mr-1 h-4 w-4" />} Save</Button>
          </DialogFooter>
