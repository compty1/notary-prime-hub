...
export default function NotebookConfigurator() {
  ...
  const [config, setConfig] = useState({ title: "", size: SIZES[0], binding: BINDINGS[0], paper: PAPERS[0], cover: COVERS[0], pageCount: 100, coverColor: "#1a1a2e", foilStamping: false, ribbonBookmark: false, elasticClosure: false, penLoop: false, quantity: 25 });
      ...
      <main className="flex-1 container mx-auto px-4 py-8 max-w-6xl">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="space-y-4">
            <Card><CardHeader><CardTitle>Preview</CardTitle></CardHeader><CardContent className="flex items-center justify-center min-h-[300px]">
              <div className="relative w-40 h-56 rounded-r-lg shadow-xl overflow-hidden" style={{ backgroundColor: config.coverColor, transform: "rotateY(-8deg)" }}>
                ...
                <div className="absolute top-8 left-6 right-4 space-y-2"><div className="h-3 bg-white/30 rounded w-3/4" /><div className="h-2 bg-white/20 rounded w-1/2" /></div>
          ...
          <div className="space-y-4">
            <Card><CardHeader><CardTitle>Configuration</CardTitle></CardHeader><CardContent className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3"><div><Label>Size</Label><Select value={config.size} onValueChange={v => update("size", v)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{SIZES.map(s => <SelectItem key={s} value={s}>{s}</SelectItem>)}</SelectContent></Select></div><div><Label>Pages</Label><Input type="number" value={config.pageCount} onChange={e => update("pageCount", Number(e.target.value))} min={50} max={400} step={10} /></div></div>
              <div className="grid grid-cols-2 gap-3"><div><Label>Binding</Label><Select value={config.binding} onValueChange={v => update("binding", v)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{BINDINGS.map(b => <SelectItem key={b} value={b}>{b}</SelectItem>)}</SelectContent></Select></div><div><Label>Paper</Label><Select value={config.paper} onValueChange={v => update("paper", v)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{PAPERS.map(p => <SelectItem key={p} value={p}>{p}</SelectItem>)}</SelectContent></Select></div></div>
              <div className="grid grid-cols-2 gap-3"><div><Label>Cover Type</Label><Select value={config.cover} onValueChange={v => update("cover", v)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{COVERS.map(c => <SelectItem key={c} value={c}>{c}</SelectItem>)}</SelectContent></Select></div><div><Label>Cover Color</Label><Input type="color" value={config.coverColor} onChange={e => update("coverColor", e.target.value)} /></div></div>
            </CardContent></Card>
            ...
            <Card><CardContent className="pt-4 space-y-3">
              ...
              <div className="p-3 rounded-lg bg-muted/50 text-sm space-y-1"><div className="flex justify-between"><span>Unit price:</span><span>${(unitPrice + extras).toFixed(2)}</span></div><div className="flex justify-between font-semibold"><span>Total:</span><span>${total.toFixed(2)}</span></div></div>
