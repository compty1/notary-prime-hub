    ...
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <main className="flex-1 container mx-auto px-4 py-8 max-w-6xl">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <h1 className="text-2xl font-bold text-foreground">Book Cover Designer</h1>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="space-y-4">
              ...
              <CardContent className="flex items-center justify-center min-h-[350px]">
                <div className="flex" style={{ perspective: "600px", transform: "rotateY(-5deg)" }}>
                  ...
                  <div className="w-6 h-72 flex items-center justify-center" style={{ backgroundColor: form.bgColor, borderLeft: "1px solid rgba(0,0,0,0.2)" }}>
                    <p className="text-[8px] writing-mode-vertical transform -rotate-180 whitespace-nowrap" style={{ color: form.textColor, writingMode: "vertical-rl" }}>
                      {form.title || "Book Title"} — {form.author || "Author"}
                    </p>
                  </div>
                  {/* Front */}
                  <div className="w-48 h-72 rounded-r-md shadow-xl overflow-hidden relative" style={{ backgroundColor: form.bgColor }}>
                    <div className="absolute inset-0 p-6 flex flex-col justify-between">
                      <div>
                        <p className="text-lg font-bold leading-tight" style={{ color: form.textColor }}>{form.title || "Book Title"}</p>
                        {form.subtitle && <p className="text-xs mt-1 opacity-70" style={{ color: form.textColor }}>{form.subtitle}</p>}
                      </div>
                      <p className="text-sm font-medium" style={{ color: form.textColor }}>{form.author || "Author Name"}</p>
                    </div>
                    {form.includeBarcode && (
                      <div className="absolute bottom-3 right-3 w-12 h-8 bg-white rounded flex items-center justify-center">
                        <div className="space-y-0.5">
                          {[...Array(5)].map((_, i) => <div key={i} className="h-0.5 bg-black" style={{ width: `${8 + Math.random() * 8}px` }} />)}
                        </div>
                      </div>
                    )}
                  </div>
            ...
            <div className="p-4 rounded-lg bg-muted/50 text-sm">
              <p className="font-medium mb-1">Spine Width Calculation</p>
              <p className="text-muted-foreground">{form.pageCount} pages × 0.0025" = <span className="font-semibold">{spineWidth}"</span> spine</p>
            </div>
          ...
          <div className="space-y-4">
            <Card>
              ...
              <CardContent className="space-y-4">
                <div><Label>Title</Label><Input value={form.title} onChange={e => update("title", e.target.value)} placeholder="My Amazing Book" /></div>
                <div><Label>Subtitle</Label><Input value={form.subtitle} onChange={e => update("subtitle", e.target.value)} placeholder="A compelling subtitle" /></div>
                <div><Label>Author</Label><Input value={form.author} onChange={e => update("author", e.target.value)} placeholder="Jane Smith" /></div>
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                ...
                <div className="p-3 rounded-lg bg-muted/50 text-sm">
                  <div className="flex justify-between font-semibold"><span>Est. Total:</span><span>${(form.quantity * (form.quantity >= 100 ? 6.49 : form.quantity >= 50 ? 6.98 : 19.99)).toFixed(2)}</span></div>
                </div>
