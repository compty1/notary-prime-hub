...
import { ArrowLeft, Download, ShoppingCart, Eye, Type, Palette, Image, CreditCard } from "lucide-react";
...
export default function BusinessCardDesigner() {
  ...
  const handleAddToCart = () => {
    toast({ title: "Added to cart", description: "Business cards added to your print cart." });
    navigate("/print-shop");
  };
    ...
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <main className="flex-1 container mx-auto px-4 py-8 max-w-7xl">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <h1 className="text-2xl font-bold text-foreground">Business Card Designer</h1>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="space-y-4">
            <Card>
              ...
              <CardContent>
                <div
                  className="relative w-full aspect-[3.5/2] rounded-lg border border-border shadow-xl overflow-hidden"
                  style={{ background: side === "front" ? form.primaryColor : "#ffffff", perspective: "600px" }}
                >
                  {side === "front" ? (
          ...
          <div>
            <Tabs defaultValue="content">
              ...
              <TabsContent value="content" className="space-y-4 mt-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div><Label>Phone</Label><Input value={form.phone} onChange={e => update("phone", e.target.value)} placeholder="(614) 555-0100" /></div>
                  <div><Label>Email</Label><Input value={form.email} onChange={e => update("email", e.target.value)} placeholder="jane@notardex.com" /></div>
                </div>
                <div><Label>Website</Label><Input value={form.website} onChange={e => update("website", e.target.value)} placeholder="notardex.com" /></div>
                <div><Label>Address</Label><Textarea value={form.address} onChange={e => update("address", e.target.value)} placeholder="123 Main St, Columbus, OH 43215" rows={2} /></div>
              </TabsContent>

              <TabsContent value="style" className="space-y-4 mt-4">
                <div><Label>Template</Label></div>
                <div className="grid grid-cols-3 gap-2">
                  {TEMPLATES.map(t => (
                    <button
                      key={t.id}
                      onClick={() => { update("template", t.id); update("primaryColor", t.colors[0]); update("accentColor", t.colors[1]); }}
              ...
              <TabsContent value="specs" className="space-y-4 mt-4">
                <div>
                  ...
                  <Select value={form.paper} onValueChange={v => update("paper", v)}>
                    <SelectTrigger><SelectValue /></SelectTrigger>
                    <SelectContent>{PAPER_OPTIONS.map(p => <SelectItem key={p} value={p}>{p}</SelectItem>)}</SelectContent>
                  </Select>
                </div>
                <div>
                  <Label>Finish / Special Effects</Label>
                  <Select value={form.finish} onValueChange={v => update("finish", v)}>
                    <SelectTrigger><SelectValue /></SelectTrigger>
                    <SelectContent>{FINISH_OPTIONS.map(f => <SelectItem key={f} value={f}>{f}</SelectItem>)}</SelectContent>
                  </Select>
                </div>
            ...
            <div className="flex gap-3 mt-6">
              <Button onClick={handleAddToCart} className="flex-1"><ShoppingCart className="h-4 w-4 mr-2" />Add to Cart</Button>
