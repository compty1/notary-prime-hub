            ...
            {modules.map((m: any) => (
              <TableRow key={m.id}>
                ...
                <TableCell className="text-right">
                  <Button variant="ghost" size="icon" onClick={() => { setEditId(m.id); setForm({ title: m.title, description: m.description || "", sort_order: String(m.sort_order), duration_minutes: String(m.duration_minutes), is_published: m.is_published }); setShowDialog(true); }} aria-label="Action"><Edit className="h-4 w-4" /></Button>
                  <Button variant="ghost" size="icon" onClick={() => del.mutate(m.id)} aria-label="Action"><Trash2 className="h-4 w-4 text-destructive" /></Button>
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
          ...
          <TableBody>
            {lessons.map((l: any) => (
              <TableRow key={l.id}>
                ...
                <TableCell className="text-right">
                  <Button variant="ghost" size="icon" onClick={() => { setEditId(l.id); setForm({ module_id: l.module_id, title: l.title, content_html: l.content_html || "", sort_order: String(l.sort_order), duration_minutes: String(l.duration_minutes), is_published: l.is_published }); setShowDialog(true); }} aria-label="Action"><Edit className="h-4 w-4" /></Button>
                  <Button variant="ghost" size="icon" onClick={() => del.mutate(l.id)} aria-label="Action"><Trash2 className="h-4 w-4 text-destructive" /></Button>
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
          ...
          <TableBody>
            {quizzes.map((q) => (
              <TableRow key={q.id}>
                ...
                <TableCell className="text-right">
                  <Button variant="ghost" size="icon" onClick={() => { setEditId(q.id); setForm({ title: q.title, quiz_type: q.quiz_type, module_id: q.module_id || "", passing_score: String(q.passing_score), sort_order: String(q.sort_order), questions: JSON.stringify(q.questions, null, 2) }); setShowDialog(true); }} aria-label="Action"><Edit className="h-4 w-4" /></Button>
                  <Button variant="ghost" size="icon" onClick={() => del.mutate(q.id)} aria-label="Action"><Trash2 className="h-4 w-4 text-destructive" /></Button>
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
