                ...
                {filtered.map((e: any) => (
                  <TableRow key={e.id} className={!e.is_active ? "opacity-50" : ""}>
                    ...
                    <TableCell className="text-right">
                      <Button variant="ghost" size="icon" onClick={() => openEdit(e)} aria-label="Action"><Edit className="h-4 w-4" /></Button>
                      <Button variant="ghost" size="icon" onClick={() => deleteMutation.mutate(e.id)} aria-label="Action"><Trash2 className="h-4 w-4 text-destructive" /></Button>
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
