...
import {
  Mail, Plus, Send, Archive, Loader2, Eye, Reply, Search, Star,
  Inbox, FileText, Trash2, PenLine, RefreshCw, MailOpen, StarOff,
...
export default function AdminEmailManagement() {
  ...
  // Profiles for autocomplete
  const [profiles, setProfiles] = useState<any[]>([]);

  // Correspondence (legacy view)
  const [correspondenceItems, setCorrespondenceItems] = useState<any[]>([]);
  ...
  const handleStarToggle = async (email: EmailItem, e: React.MouseEvent) => {
    ...
    try {
      await callEdgeFunction("ionos-email", { action, id: email.id });
  ...
  const handleBulkAction = async (bulkAction: string) => {
    ...
    try {
      await callEdgeFunction("ionos-email", {
        action: "bulk_action",
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      await supabase.from("email_drafts").insert({
        user_id: user.id,
        to_addresses: composeTo ? [composeTo] : [],
        cc_addresses: composeCc ? [composeCc] : [],
        subject: composeSubject,
        body_html: composeBody,
        in_reply_to: composeInReplyTo || null,
      } as any);
    ...
    <div className="flex flex-col h-[calc(100vh-8rem)]">
      ...
      <div className="mb-4 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">IONOS email client — send, receive, and manage correspondence</p>
        </div>
      ...
      {/* Main Layout */}
      <div className="flex flex-1 gap-4 overflow-hidden rounded-lg border border-border">
        {/* Folder Sidebar */}
          ...
          <div className="border-b border-border p-3 space-y-2">
            ...
            {selectedIds.size > 0 && (
              <div className="flex items-center gap-2 text-xs">
                ...
                <Button variant="ghost" size="sm" className="h-6 text-xs" onClick={() => handleBulkAction("mark_read")}>
                  <MailOpen className="mr-1 h-3 w-3" /> Read
                </Button>
                <Button variant="ghost" size="sm" className="h-6 text-xs" onClick={() => handleBulkAction("archive")}>
            ...
            <div className="border-b border-border p-4">
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex gap-1">
                  <Button variant="ghost" size="sm" onClick={() => openCompose("reply", selectedEmail)} title="Reply">
                    <Reply className="h-4 w-4" />
                  </Button>
                  <Button variant="ghost" size="sm" onClick={() => openCompose("replyAll", selectedEmail)} title="Reply All">
                    <ReplyAll className="h-4 w-4" />
                  </Button>
                  <Button variant="ghost" size="sm" onClick={() => openCompose("forward", selectedEmail)} title="Forward">
                    <Forward className="h-4 w-4" />
                  </Button>
                  <Button variant="ghost" size="sm" onClick={() => handleArchive(selectedEmail.id)} title="Archive">
                    <Archive className="h-4 w-4" />
                  </Button>
                  <Button variant="ghost" size="sm" onClick={() => handleDelete(selectedEmail.id)} title="Delete" className="text-destructive hover:text-destructive">
      ...
      <Dialog open={showSignatures} onOpenChange={setShowSignatures}>
        <DialogContent className="sm:max-w-lg max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="font-sans">Email Signatures</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
              ...
              <Card key={sig.id} className="border-border/50">
                <CardContent className="p-3">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="flex gap-1">
                      {!sig.is_default && (
                        <Button variant="ghost" size="sm" className="text-xs h-7" onClick={() => handleSetDefaultSig(sig.id)}>Set Default</Button>
                      )}
            ...
            <div className="space-y-3">
              ...
              <div>
                <Label>Name</Label>
                <Input value={sigName} onChange={e => setSigName(e.target.value)} placeholder="e.g., Professional" />
              </div>
              <div>
                <Label>Signature Content</Label>
                <RichTextEditor value={sigHtml} onChange={setSigHtml} placeholder="Your email signature..." minHeight="100px" className="mt-1" />
              </div>
              <div className="flex gap-2">
                <Button size="sm" onClick={handleSaveSignature} disabled={!sigName.trim()}>
                  {editingSigId ? "Update" : "Create"} Signature
                </Button>
                {editingSigId && (
                  <Button variant="outline" size="sm" onClick={() => { setEditingSigId(null); setSigName(""); setSigHtml(""); }}>Cancel</Button>
                )}
              </div>
            </div>
          </div>
