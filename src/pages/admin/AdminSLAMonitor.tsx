...
export default function AdminSLAMonitor() {
  ...
  const { data: orders = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("orders").select("id, status, created_at, updated_at")
        .order("created_at", { ascending: false }).limit(200);
      return (data || []) as { id: string; status: string; created_at: string; scheduled_date?: string }[];
    },
  });
  ...
  const slaMetrics = Object.entries(SLA_TARGETS).map(([service, target]) => {
    ...
    const allItems = [...serviceAppts] as { id: string; status: string; created_at: string; scheduled_date?: string }[];
