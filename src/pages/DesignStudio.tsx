...
import { Search, Palette, PenTool, BookOpen, Sticker, NotebookPen, CreditCard, Shirt, Bookmark, FileText } from "lucide-react";

const DESIGN_TOOLS = [
  { id: "business-cards", label: "Business Card Designer", description: "Design professional business cards with templates, custom fonts, and instant preview", icon: CreditCard, route: "/design/business-cards", productType: "business-cards" },
  { id: "stickers", label: "Sticker Designer", description: "Create custom die-cut stickers, labels, and decals with shape tools", icon: Sticker, route: "/design/stickers", productType: "stickers" },
  { id: "notebooks", label: "Notebook Configurator", description: "Design custom notebook covers, choose binding, paper type, and page count", icon: NotebookPen, route: "/design/notebooks", productType: "notebooks" },
  { id: "book-covers", label: "Book Cover Designer", description: "Create stunning book covers with spine calculation and barcode placement", icon: BookOpen, route: "/design/book-covers", productType: "books" },
  ...
  { id: "signage", label: "Sign & Banner Builder", description: "Create banners, yard signs, and window graphics with size presets", icon: Bookmark, route: "/design/signage", productType: "signage" },
...
export default function DesignStudio() {
  ...
  const filtered = DESIGN_TOOLS.filter(t =>
      ...
      <main className="flex-1 container mx-auto px-4 py-8 max-w-7xl">
          ...
          {filtered.map(tool => (
            <Card key={tool.id} className="group hover:shadow-lg transition-all cursor-pointer border-border/60 hover:border-primary/40" onClick={() => navigate(tool.route)}>
              <CardContent className="p-0">
                <div className="p-4">
                  <ProductPreview3D productType={tool.productType} className="h-40 mb-4" />
                </div>
        ...
        {filtered.length === 0 && (
          <div className="text-center py-16 text-muted-foreground">
            <PenTool className="h-12 w-12 mx-auto mb-3 opacity-40" />
            <p>No design tools match your search.</p>
          </div>
        )}
      </main>
      <Footer />
