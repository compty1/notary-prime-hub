        ...
        <Tabs defaultValue="active">
          ...
          <TabsContent value="active">
            <Card>
              <CardContent className="p-0">
                <Table>
                  <TableHeader><TableRow><TableHead>Order #</TableHead><TableHead>Product</TableHead><TableHead>Qty</TableHead><TableHead>Status</TableHead><TableHead>Due</TableHead><TableHead>Total</TableHead></TableRow></TableHeader>
                  <TableBody>
                    {isLoading ? (
                      <TableRow><TableCell colSpan={6} className="text-center py-8">Loading...</TableCell></TableRow>
                    ) : activeOrders.length === 0 ? (
                      <TableRow><TableCell colSpan={6} className="text-center py-8 text-muted-foreground">No active orders</TableCell></TableRow>
                      ...
                      <TableRow key={o.id}>
                        ...
                        <TableCell><Badge className={STATUS_COLORS[o.status] || ""}>{o.status.replace(/_/g, " ")}</Badge></TableCell>
          ...
          <TabsContent value="completed">
            <Card>
              <CardContent className="p-0">
                <Table>
                  <TableHeader><TableRow><TableHead>Order #</TableHead><TableHead>Product</TableHead><TableHead>Qty</TableHead><TableHead>Delivered</TableHead><TableHead>Total</TableHead></TableRow></TableHeader>
                  <TableBody>
                    {completedOrders.length === 0 ? (
                      <TableRow><TableCell colSpan={5} className="text-center py-8 text-muted-foreground">No completed orders</TableCell></TableRow>
                    ) : completedOrders.map((o: any) => (
                      <TableRow key={o.id}>
                        <TableCell className="font-mono text-sm">{o.order_number || o.id.slice(0, 8)}</TableCell>
                        <TableCell>{o.product_type}</TableCell>
                        <TableCell>{o.quantity}</TableCell>
                        <TableCell>{o.updated_at ? new Date(o.updated_at).toLocaleDateString() : "—"}</TableCell>
                        <TableCell>${Number(o.total_price || 0).toFixed(2)}</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
          ...
          <TabsContent value="messages">
            <Card>
              <CardContent className="py-12 text-center text-muted-foreground">
                ...
                <p className="font-medium">Order Communications</p>
                <p className="text-sm">Select an order to view or send messages.</p>
              </CardContent>
            </Card>
          </TabsContent>
        </Tabs>
