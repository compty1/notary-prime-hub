...
import heroSmallBusiness from "@/assets/hero-solutions-small-business.png";
import heroSmallBusinessAvif from "@/assets/hero-solutions-small-business.avif";
import heroSmallBusinessWebp from "@/assets/hero-solutions-small-business.webp";
          ...
          <div className="grid items-center gap-12 md:grid-cols-2">
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }}>
              ...
              <div className="flex flex-wrap gap-3">
                ...
                <Link to="/services?category=business"><Button size="lg" variant="outline" className="rounded-full px-8 border-2 border-foreground bg-card text-foreground shadow-block hover:-translate-y-0.5 hover:shadow-block-lg font-black uppercase tracking-tight transition-all">Business Services</Button></Link>
              </div>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.5, delay: 0.2 }} className="flex justify-center">
              <div className="relative">
                <Picture src={heroSmallBusiness} alt="Small business notarization — paper-card illustration" fetchPriority="high" decoding="async" sizes="(max-width: 768px) 90vw, 420px" width={1024} height={1024} className="mx-auto h-auto w-full max-w-[420px] drop-shadow-xl" loading="eager" sources={{ avif: heroSmallBusinessAvif, webp: heroSmallBusinessWebp }} />
                <p className="mt-4 text-center text-lg font-semibold text-foreground">Built for Business</p>
                <p className="mt-1 text-center text-sm text-muted-foreground">Subscription plans from $29/mo</p>
                <div className="absolute -bottom-1 -right-1 rounded-full bg-accent px-4 py-2 text-xs font-bold text-accent-foreground shadow-lg">Volume Discounts</div>
              </div>
            </motion.div>
          </div>
        </div>
