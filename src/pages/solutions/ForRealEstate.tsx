...
import { Home, FileText, Monitor, Shield, Users, DollarSign, ArrowRight, CheckCircle } from "lucide-react";

const features = [
  { icon: Home, title: "Closing Documents", desc: "Deeds, mortgages, settlement statements, and all closing documents notarized on-site or via RON for seamless transactions." },
  { icon: FileText, title: "Deed Transfers & Title Work", desc: "Warranty deeds, quitclaim deeds, title affidavits, and lien releases notarized with proper Ohio recording requirements." },
  { icon: DollarSign, title: "Mortgage & Refinance", desc: "Complete loan signing packages — refinances, HELOCs, reverse mortgages, and modifications with certified signing agent services." },
  { icon: Monitor, title: "RON for Remote Closings", desc: "Ohio-authorized Remote Online Notarization for out-of-state buyers, sellers, or anyone who can't attend in person." },
  { icon: Users, title: "Multi-Signer Coordination", desc: "We coordinate schedules for transactions involving multiple signers, witnesses, and parties across different locations." },
  { icon: Shield, title: "Bulk Signing Packages", desc: "Volume pricing for title companies, brokerages, and lenders with recurring notarization needs. Dedicated account management." },
];
          ...
          <div className="grid items-center gap-12 md:grid-cols-2">
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }}>
              ...
              <p className="mb-6 text-lg text-muted-foreground">
                From purchase agreements to refinance packages, we handle the full spectrum of real estate
                notarization — on-site at the closing table or remotely via Ohio-authorized RON.
              </p>
              <div className="flex flex-wrap gap-3">
                <Link to="/book">
                  <Button size="lg" className="rounded-full px-8">
                    Schedule a Closing <ArrowRight className="ml-2 h-4 w-4" />
                  </Button>
                </Link>
                <Link to="/loan-signing">
                  <Button size="lg" variant="outline">Loan Signing Services</Button>
                </Link>
              </div>
            </motion.div>
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.5, delay: 0.2 }} className="flex justify-center">
              <div className="relative rounded-2xl border border-border bg-card p-8 shadow-lg">
                <Home className="mx-auto mb-4 h-16 w-16 text-primary" />
