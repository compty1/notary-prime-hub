...
Be extremely detailed and include safety/compliance warnings where appropriate.`,
  ...
  },
  {
    ...
    icon: Crosshair,
    fields: [
      { name: "yourBusiness", label: "Your Business", type: "text", required: true, placeholder: "Notar" },
...
Be analytical, data-driven, and provide actionable insights.`,
  ...
  },
  {
    ...
    icon: Newspaper,
    fields: [
      { name: "headline", label: "Headline / Announcement", type: "text", required: true, placeholder: "Notar Launches AI-Powered Document Review" },
      { name: "company", label: "Company Name", type: "text", required: true, placeholder: "Notar" },
...
Be thorough, specific, and prioritize practical mitigation strategies.`,
  ...
  },
  {
    ...
    icon: Megaphone,
    fields: [
      { name: "brandName", label: "Brand Name", type: "text", required: true, placeholder: "Notar" },
...
Be specific, include plenty of examples, and make the guide immediately actionable.`,
  ...
  // ═══════════════════════════════════════
  {
    id: "ux-heuristic-audit",
    title: "UX Heuristic Audit",
    category: "UX Consulting",
    description: "Evaluate a product against Nielsen's 10 usability heuristics with severity ratings and fix recommendations.",
    icon: Search,
    fields: [
      { name: "productUrl", label: "Product / URL / Description", type: "textarea", placeholder: "Describe the product, page, or paste a URL...", required: true },
      { name: "industry", label: "Industry", type: "text", placeholder: "e.g., FinTech, Healthcare" },
    ],
    systemPrompt: `You are a senior UX consultant performing a heuristic evaluation based on Nielsen's 10 usability heuristics. For each heuristic, assess the product, assign a severity rating (0-4), describe specific issues found, and provide actionable fix recommendations. Include a summary scorecard table at the top. Output in detailed markdown.`,
    maxTokens: 6000,
  },
  {
    id: "ux-user-journey-map",
    title: "User Journey Map Generator",
    category: "UX Consulting",
    description: "Create a comprehensive user journey map with touchpoints, emotions, pain points, and opportunities.",
    icon: Workflow,
    fields: [
      { name: "persona", label: "User Persona", type: "textarea", placeholder: "Describe the user...", required: true },
      { name: "scenario", label: "Scenario / Goal", type: "text", placeholder: "e.g., First-time purchase" },
      { name: "product", label: "Product / Service", type: "text", placeholder: "e.g., E-commerce platform" },
    ],
    systemPrompt: `You are a UX researcher. Create a detailed user journey map in markdown with phases, touchpoints, user actions, thoughts, emotions (use emoji scale), pain points, and opportunities. Include a summary of key moments of truth and recommended improvements. Format as a table where possible.`,
    maxTokens: 5000,
  },
  {
    ...
    category: "UX Consulting",
    description: "Audit against WCAG 2.1 AA guidelines with prioritized remediation plan.",
    icon: UserCheck,
    fields: [
      { name: "target", label: "Page / Component Description", type: "textarea", placeholder: "Describe the UI or paste markup...", required: true },
      { name: "level", label: "Target Level", type: "select", options: [
        { value: "A", label: "Level A" }, { value: "AA", label: "Level AA" }, { value: "AAA", label: "Level AAA" },
      ]},
    ],
    systemPrompt: `You are a WCAG accessibility expert. Perform a thorough audit against WCAG 2.1 at the specified level. For each criterion, mark pass/fail, describe the issue, provide the WCAG reference number, and suggest a specific fix. Include a prioritized remediation plan sorted by impact. Output in markdown with tables.`,
    maxTokens: 6000,
  },
  {
    id: "ux-information-architecture",
    title: "Information Architecture Planner",
    category: "UX Consulting",
    description: "Design a site map, navigation structure, and content taxonomy.",
    icon: GitBranch,
    fields: [
      { name: "project", label: "Project / Site Description", type: "textarea", placeholder: "Describe the product and its content...", required: true },
      { name: "userTypes", label: "Primary User Types", type: "text", placeholder: "e.g., Consumers, Admins, Partners" },
    ],
    systemPrompt: `You are an information architecture specialist. Create a comprehensive IA plan including: site map hierarchy (use indented list), primary/secondary/utility navigation structure, content taxonomy, labeling system recommendations, and search strategy. Include a card sorting recommendation and tree testing plan. Output in detailed markdown.`,
    maxTokens: 5000,
  },
  {
    id: "ux-design-system-audit",
    title: "Design System Audit",
    category: "UX Consulting",
    description: "Evaluate design system consistency, coverage, and token architecture.",
    icon: Layers,
    fields: [
      { name: "components", label: "Component List / Description", type: "textarea", placeholder: "List key components or describe the design system...", required: true },
      { name: "platform", label: "Platform", type: "select", options: [
        { value: "web", label: "Web" }, { value: "mobile", label: "Mobile" }, { value: "both", label: "Cross-platform" },
      ]},
    ],
    systemPrompt: `You are a design systems expert. Audit the described design system for: token completeness (color, typography, spacing, elevation), component coverage gaps, naming convention consistency, accessibility compliance, documentation quality, and versioning strategy. Provide a maturity scorecard and improvement roadmap. Output in markdown.`,
    maxTokens: 5000,
  },
  {
    id: "ux-usability-test-plan",
    title: "Usability Test Plan",
    category: "UX Consulting",
    description: "Generate a complete usability test plan with tasks, metrics, and script.",
    icon: ClipboardCheck,
    fields: [
      { name: "product", label: "Product Description", type: "textarea", placeholder: "What are you testing?", required: true },
      { name: "goals", label: "Research Goals", type: "textarea", placeholder: "What do you want to learn?" },
      { name: "method", label: "Method", type: "select", options: [
        { value: "moderated", label: "Moderated" }, { value: "unmoderated", label: "Unmoderated" }, { value: "guerrilla", label: "Guerrilla" },
      ]},
    ],
    systemPrompt: `You are a UX researcher. Create a comprehensive usability test plan including: objectives, methodology, participant criteria and recruitment plan, 5-8 task scenarios with success criteria, pre/post-test questions, moderator script, metrics to collect (task completion, time, errors, SUS score), analysis plan, and timeline. Output in markdown.`,
    maxTokens: 5000,
  },
  {
    ...
    category: "UX Consulting",
    description: "Analyze a page/flow for conversion barriers and generate improvement hypotheses.",
    icon: TrendingUp,
    fields: [
      { name: "page", label: "Page / Flow Description", type: "textarea", placeholder: "Describe the page, funnel, or user flow...", required: true },
      { name: "conversionGoal", label: "Conversion Goal", type: "text", placeholder: "e.g., Sign-up, Purchase, Lead capture" },
      { name: "currentRate", label: "Current Conversion Rate (optional)", type: "text", placeholder: "e.g., 2.3%" },
    ],
    systemPrompt: `You are a CRO specialist. Analyze the described page/flow for conversion barriers. Output a prioritized list of issues using the PIE framework (Potential, Importance, Ease). For each issue, provide: the barrier, psychological principle at play, specific fix recommendation, expected impact, and implementation difficulty. Include A/B test hypotheses. Output in markdown.`,
    maxTokens: 5000,
  },
  {
    id: "ux-micro-copy",
    ...
    category: "UX Consulting",
    description: "Generate UX microcopy for buttons, errors, empty states, tooltips, and onboarding.",
    icon: Pencil,
    fields: [
      { name: "context", label: "Context / Screen Description", type: "textarea", placeholder: "Describe the UI context...", required: true },
      { name: "tone", label: "Brand Tone", type: "select", options: [
        { value: "professional", label: "Professional" }, { value: "friendly", label: "Friendly" },
        { value: "playful", label: "Playful" }, { value: "minimal", label: "Minimal" },
      ]},
      { name: "copyType", label: "Copy Type", type: "select", options: [
        { value: "buttons", label: "CTAs & Buttons" }, { value: "errors", label: "Error Messages" },
        { value: "empty", label: "Empty States" }, { value: "tooltips", label: "Tooltips" },
        { value: "onboarding", label: "Onboarding" }, { value: "all", label: "Full Set" },
      ]},
    ],
    systemPrompt: `You are a UX writer specializing in microcopy. Generate clear, helpful, and on-brand microcopy for the requested type. For each piece, provide: the copy text, rationale, character count, and accessibility note. Include do/don't examples. Output in markdown tables.`,
    maxTokens: 4000,
  },
  {
    id: "ux-competitive-ux-analysis",
    title: "Competitive UX Analysis",
    category: "UX Consulting",
    description: "Compare UX patterns across competitors with feature matrix and gap analysis.",
    icon: Crosshair,
    fields: [
      { name: "product", label: "Your Product", type: "text", placeholder: "e.g., NotarDex", required: true },
      { name: "competitors", label: "Competitors (comma-separated)", type: "text", placeholder: "e.g., Notarize, PandaDoc, DocuSign" },
      { name: "focusAreas", label: "Focus Areas", type: "textarea", placeholder: "e.g., Onboarding, Pricing page, Mobile experience" },
    ],
    systemPrompt: `You are a UX strategist. Perform a competitive UX analysis covering: feature comparison matrix, UX pattern analysis (navigation, onboarding, checkout), strengths/weaknesses per competitor, differentiation opportunities, best practices to adopt, and strategic recommendations. Include a scoring table. Output in detailed markdown.`,
    maxTokens: 6000,
  },
  {
    id: "ux-wireframe-spec",
    title: "Wireframe Specification",
    category: "UX Consulting",
    description: "Generate detailed wireframe specifications with component descriptions and interaction notes.",
    icon: LayoutGrid,
    fields: [
      { name: "screen", label: "Screen / Page Name", type: "text", placeholder: "e.g., Dashboard, Settings, Checkout", required: true },
      { name: "requirements", label: "Requirements", type: "textarea", placeholder: "What should this screen accomplish?" },
      { name: "platform", label: "Platform", type: "select", options: [
        { value: "desktop", label: "Desktop" }, { value: "mobile", label: "Mobile" }, { value: "responsive", label: "Responsive" },
      ]},
    ],
    systemPrompt: `You are a UX designer. Create a detailed wireframe specification including: layout grid definition, component inventory with descriptions, content hierarchy, interaction states (default, hover, active, error, loading, empty), responsive breakpoint notes, accessibility requirements per component, and developer handoff notes. Use ASCII art for basic layout representation. Output in markdown.`,
    maxTokens: 5000,
  },
  {
    id: "ux-design-critique",
    title: "Design Critique Report",
    category: "UX Consulting",
    description: "Structured design critique with actionable feedback on visual and interaction design.",
    icon: AlertTriangle,
    fields: [
      { name: "design", label: "Design Description / Screenshot Notes", type: "textarea", placeholder: "Describe the design to critique...", required: true },
      { name: "criteria", label: "Focus Criteria", type: "text", placeholder: "e.g., Visual hierarchy, Color contrast, Typography" },
    ],
    systemPrompt: `You are a senior design critic. Provide a structured design critique covering: visual hierarchy, typography system, color usage and accessibility, spacing and alignment, component consistency, interaction design, content strategy, and emotional design. Rate each area 1-5, provide specific issues with screenshot references, and suggest fixes with rationale. Output in markdown.`,
    maxTokens: 5000,
  },
  {
    ...
    category: "UX Consulting",
    description: "Design an optimized user onboarding flow with progressive disclosure and activation metrics.",
    icon: Rocket,
    fields: [
      { name: "product", label: "Product Description", type: "textarea", placeholder: "Describe your product...", required: true },
      { name: "userType", label: "Primary User Type", type: "text", placeholder: "e.g., Small business owner" },
      { name: "ahaMetric", label: "'Aha' Moment", type: "text", placeholder: "e.g., First document notarized" },
    ],
    systemPrompt: `You are an onboarding UX specialist. Design a complete onboarding flow including: welcome screen copy, progressive disclosure steps (max 5), required vs. optional fields per step, activation milestones, tooltip/coach mark placements, email drip sequence triggers, success metrics (activation rate, time-to-value), falloff prevention tactics, and re-engagement strategy. Output in markdown.`,
    maxTokens: 5000,
  },
  {
    id: "ux-form-optimizer",
    title: "Form UX Optimizer",
    category: "UX Consulting",
    description: "Analyze and optimize form design for completion rates and user experience.",
    icon: ListChecks,
    fields: [
      { name: "formDescription", label: "Form Description / Fields", type: "textarea", placeholder: "List the form fields and purpose...", required: true },
      { name: "context", label: "Context", type: "text", placeholder: "e.g., Checkout, Registration, Booking" },
    ],
    systemPrompt: `You are a form UX expert. Analyze the form and provide: field reduction recommendations, optimal field ordering, input type optimization, inline validation strategy, error message copy, progress indication approach, mobile optimization notes, autofill/autocomplete attributes, and accessibility checklist. Include before/after comparison. Output in markdown.`,
    maxTokens: 4000,
  },
  {
    id: "ux-persona-builder",
    title: "Research-Backed Persona Builder",
    category: "UX Consulting",
    description: "Create detailed user personas with behavioral patterns, goals, and design implications.",
    icon: UserPlus,
    fields: [
      { name: "product", label: "Product / Service", type: "text", placeholder: "e.g., Online notarization platform", required: true },
      { name: "segment", label: "User Segment", type: "text", placeholder: "e.g., First-time home buyers" },
      { name: "dataPoints", label: "Known Data Points (optional)", type: "textarea", placeholder: "Any research findings, survey data, analytics..." },
    ],
    systemPrompt: `You are a UX researcher. Create a detailed user persona including: demographic summary, behavioral patterns (tools used, frequency, context), goals and motivations, frustrations and pain points, technology comfort level, information needs, decision-making factors, quote that captures their mindset, scenarios of use, and design implications. Make it research-grounded, not stereotypical. Output in markdown.`,
    maxTokens: 4000,
  },
  {
    id: "ux-mobile-audit",
    ...
    category: "UX Consulting",
    description: "Evaluate mobile experience for touch targets, performance, gestures, and platform conventions.",
    icon: Zap,
    fields: [
      { name: "app", label: "App / Mobile Site Description", type: "textarea", placeholder: "Describe the mobile experience...", required: true },
      { name: "platform", label: "Platform", type: "select", options: [
        { value: "ios", label: "iOS" }, { value: "android", label: "Android" }, { value: "pwa", label: "PWA / Mobile Web" },
      ]},
    ],
    systemPrompt: `You are a mobile UX specialist. Audit the mobile experience for: touch target sizing (44pt minimum), gesture support, thumb zone optimization, platform convention adherence (HIG/Material), loading performance, offline capability, form input optimization, notification strategy, and accessibility. Rate each area and provide specific fixes. Output in markdown.`,
    maxTokens: 5000,
  },
  ...
  // ═══════════════════════════════════════
  {
    ...
    description: "Project cash inflows and outflows over 3-12 months with scenario analysis.",
    icon: TrendingUp,
    fields: [
      { name: "revenue", label: "Monthly Revenue (current)", type: "text", placeholder: "e.g., $15,000", required: true },
      { name: "expenses", label: "Monthly Expenses", type: "text", placeholder: "e.g., $10,000" },
      { name: "growthRate", label: "Expected Growth Rate", type: "text", placeholder: "e.g., 5% monthly" },
      { name: "period", label: "Forecast Period", type: "select", options: [
        { value: "3", label: "3 months" }, { value: "6", label: "6 months" }, { value: "12", label: "12 months" },
      ]},
    ],
    systemPrompt: `You are a financial analyst. Create a detailed cash flow forecast with: monthly projection table (inflows, outflows, net, cumulative), three scenarios (optimistic, baseline, conservative), key assumptions listed, break-even analysis, risk factors, and recommendations for improving cash position. Include clear markdown tables.`,
    maxTokens: 5000,
  },
  {
    id: "fin-pricing-strategy",
    title: "Pricing Strategy Analyzer",
    category: "Financial Operations",
    description: "Evaluate and optimize pricing models with competitive positioning and elasticity analysis.",
    icon: Tag,
    fields: [
      { name: "product", label: "Product / Service", type: "text", placeholder: "e.g., Notarization services", required: true },
      { name: "currentPricing", label: "Current Pricing", type: "textarea", placeholder: "List your current prices..." },
      { name: "competitors", label: "Competitor Pricing (optional)", type: "textarea", placeholder: "Known competitor prices..." },
      { name: "costBase", label: "Cost Base", type: "text", placeholder: "e.g., $8 per transaction" },
    ],
    systemPrompt: `You are a pricing strategist. Analyze the pricing and provide: pricing model evaluation (cost-plus, value-based, competitive), price elasticity assessment, competitive positioning map, tiered pricing recommendations, bundle opportunities, discount strategy, psychological pricing tactics, margin analysis, and implementation roadmap. Output in markdown with tables.`,
    maxTokens: 5000,
  },
  {
    id: "fin-budget-template",
    title: "Business Budget Builder",
    category: "Financial Operations",
    description: "Generate a comprehensive business budget with categories, allocations, and variance tracking.",
    icon: PieChart,
    fields: [
      { name: "businessType", label: "Business Type", type: "text", placeholder: "e.g., Mobile notary service", required: true },
      { name: "annualRevenue", label: "Annual Revenue Target", type: "text", placeholder: "e.g., $200,000" },
      { name: "employees", label: "Team Size", type: "text", placeholder: "e.g., 3" },
    ],
    systemPrompt: `You are a financial planner. Create a comprehensive annual budget template with: revenue projections by category, expense categories (fixed, variable, discretionary), monthly allocation tables, percentage benchmarks for the industry, variance tracking framework, KPI definitions, and quarterly review checklist. Output in markdown with detailed tables.`,
    maxTokens: 5000,
  },
  {
    id: "fin-invoice-analyzer",
    title: "Invoice Pattern Analyzer",
    category: "Financial Operations",
    description: "Analyze invoicing patterns, identify trends, and optimize billing cycles.",
    icon: Receipt,
    fields: [
      { name: "invoiceData", label: "Invoice Summary / Patterns", type: "textarea", placeholder: "Describe your invoicing history, common amounts, payment terms...", required: true },
      { name: "issues", label: "Known Issues", type: "textarea", placeholder: "e.g., Late payments, disputes, seasonal dips" },
    ],
    systemPrompt: `You are a billing operations analyst. Analyze invoicing patterns and provide: payment trend analysis, common delay patterns, seasonal revenue insights, recommended billing cycle changes, late payment reduction strategies, invoice template optimization, automated reminder schedule, and collection escalation workflow. Output in markdown.`,
    maxTokens: 4000,
  },
  {
    id: "fin-profit-loss",
    ...
    category: "Financial Operations",
    description: "Generate a formatted Profit & Loss statement with analysis and insights.",
    icon: BarChart3,
    fields: [
      { name: "revenue", label: "Revenue Breakdown", type: "textarea", placeholder: "List revenue streams and amounts...", required: true },
      { name: "expenses", label: "Expense Breakdown", type: "textarea", placeholder: "List expense categories and amounts..." },
      { name: "period", label: "Period", type: "text", placeholder: "e.g., Q1 2026" },
    ],
    systemPrompt: `You are an accountant. Generate a professional P&L statement with: revenue section (by stream), COGS, gross profit, operating expenses (by category), EBITDA, net income, key ratios (gross margin, operating margin, net margin), year-over-year comparison framework, and executive summary with insights. Output in markdown with properly formatted tables.`,
    maxTokens: 5000,
  },
  {
    id: "fin-expense-optimizer",
    title: "Expense Optimization Report",
    category: "Financial Operations",
    description: "Identify cost reduction opportunities across business operations.",
    icon: ArrowUpDown,
    fields: [
      { name: "expenses", label: "Current Expense Categories", type: "textarea", placeholder: "List your expenses by category with approximate amounts...", required: true },
      { name: "industry", label: "Industry", type: "text", placeholder: "e.g., Professional services" },
    ],
    systemPrompt: `You are a cost optimization consultant. Analyze expenses and provide: benchmark comparison vs. industry averages, top 10 cost reduction opportunities ranked by impact, vendor consolidation recommendations, subscription audit checklist, automation opportunities to reduce labor costs, tax deduction optimization, and a 90-day action plan. Output in markdown.`,
    maxTokens: 5000,
  },
  {
    id: "fin-tax-prep-checklist",
    title: "Tax Preparation Checklist",
    category: "Financial Operations",
    description: "Generate a comprehensive tax preparation checklist with deduction categories and deadlines.",
    icon: ClipboardCheck,
    fields: [
      { name: "entityType", label: "Entity Type", type: "select", required: true, options: [
        { value: "sole_prop", label: "Sole Proprietor" }, { value: "llc", label: "LLC" },
        { value: "s_corp", label: "S-Corp" }, { value: "c_corp", label: "C-Corp" },
      ]},
      { name: "state", label: "State", type: "text", placeholder: "e.g., Ohio" },
      { name: "industry", label: "Industry", type: "text", placeholder: "e.g., Notary services" },
    ],
    systemPrompt: `You are a tax preparation specialist. Generate a comprehensive tax prep checklist including: document gathering list, income documentation, deduction categories specific to the entity type and industry, estimated tax payment schedule, state-specific requirements, common audit triggers to avoid, recordkeeping best practices, and key deadlines. Output in markdown.`,
    maxTokens: 5000,
  },
  {
    id: "fin-kpi-dashboard",
    title: "Financial KPI Dashboard Design",
    category: "Financial Operations",
    description: "Design a financial KPI dashboard with metrics, targets, and alert thresholds.",
    icon: BarChart3,
    fields: [
      { name: "businessType", label: "Business Type", type: "text", placeholder: "e.g., Service business", required: true },
      { name: "stage", label: "Business Stage", type: "select", options: [
        { value: "startup", label: "Startup" }, { value: "growth", label: "Growth" }, { value: "mature", label: "Mature" },
      ]},
    ],
    systemPrompt: `You are a financial analytics expert. Design a KPI dashboard with: 15-20 key metrics organized by category (revenue, profitability, liquidity, efficiency, growth), target values with benchmarks, red/yellow/green thresholds, calculation formulas, data sources, update frequency, and executive summary format. Output in markdown with tables.`,
    maxTokens: 5000,
  },
  {
    ...
    category: "Financial Operations",
    description: "Design and validate revenue models with unit economics and growth projections.",
    icon: Rocket,
    fields: [
      { name: "product", label: "Product / Service", type: "textarea", placeholder: "Describe what you sell...", required: true },
      { name: "currentRevenue", label: "Current Revenue (optional)", type: "text", placeholder: "e.g., $10K/month" },
      { name: "targetRevenue", label: "Revenue Target", type: "text", placeholder: "e.g., $50K/month in 12 months" },
    ],
    systemPrompt: `You are a revenue strategist. Build a comprehensive revenue model including: revenue stream identification, unit economics (CAC, LTV, payback period), pricing tiers, volume projections, channel mix, upsell/cross-sell opportunities, churn impact analysis, seasonal adjustments, growth lever prioritization, and monthly projection table. Output in markdown.`,
    maxTokens: 5000,
  },
  {
    id: "fin-vendor-comparison",
    title: "Vendor Cost Comparison",
    category: "Financial Operations",
    description: "Create a structured vendor comparison with TCO analysis and scoring matrix.",
    icon: Scale,
    fields: [
      { name: "category", label: "Vendor Category", type: "text", placeholder: "e.g., Payment processing, Cloud hosting", required: true },
      { name: "vendors", label: "Vendors to Compare", type: "text", placeholder: "e.g., Stripe, Square, PayPal" },
      { name: "requirements", label: "Key Requirements", type: "textarea", placeholder: "List must-haves and nice-to-haves..." },
    ],
    systemPrompt: `You are a procurement analyst. Create a vendor comparison including: feature comparison matrix, pricing breakdown (setup, monthly, per-transaction, hidden fees), total cost of ownership for 1 and 3 years, weighted scoring matrix, integration complexity, support quality, contract terms, migration risk, and recommendation with rationale. Output in markdown with tables.`,
    maxTokens: 5000,
  },
  {
    id: "fin-collections-playbook",
    title: "Collections Playbook",
    category: "Financial Operations",
    description: "Generate an accounts receivable collections strategy with escalation workflows.",
    icon: DollarSign,
    fields: [
      { name: "businessType", label: "Business Type", type: "text", placeholder: "e.g., Professional services", required: true },
      { name: "avgInvoice", label: "Average Invoice Amount", type: "text", placeholder: "e.g., $500" },
      { name: "terms", label: "Payment Terms", type: "text", placeholder: "e.g., Net 30" },
    ],
    systemPrompt: `You are an AR collections specialist. Create a collections playbook with: payment terms optimization, automated reminder schedule (email templates at each stage), phone script for overdue calls, escalation timeline (30/60/90 days), negotiation tactics for payment plans, legal escalation criteria, write-off thresholds, and prevention strategies. Include email templates. Output in markdown.`,
    maxTokens: 5000,
  },
  {
    id: "fin-financial-health-check",
    title: "Financial Health Check",
    category: "Financial Operations",
    description: "Comprehensive business financial health assessment with scoring and recommendations.",
    icon: Heart,
    fields: [
      { name: "revenue", label: "Annual Revenue", type: "text", placeholder: "e.g., $200,000", required: true },
      { name: "expenses", label: "Annual Expenses", type: "text", placeholder: "e.g., $150,000" },
      { name: "cash", label: "Cash on Hand", type: "text", placeholder: "e.g., $30,000" },
      { name: "debt", label: "Total Debt", type: "text", placeholder: "e.g., $20,000" },
    ],
    systemPrompt: `You are a financial advisor. Perform a comprehensive financial health check including: profitability score, liquidity analysis (current ratio, quick ratio), debt-to-equity assessment, cash runway calculation, revenue diversification grade, expense efficiency rating, growth sustainability index, emergency fund adequacy, and overall health score (A-F). Provide specific improvement actions for each area. Output in markdown.`,
    maxTokens: 5000,
  },
  {
    id: "fin-grant-budget",
    ...
    category: "Financial Operations",
    description: "Create a detailed grant budget narrative with justifications for each line item.",
    icon: Landmark,
    fields: [
      { name: "grantAmount", label: "Grant Amount", type: "text", placeholder: "e.g., $50,000", required: true },
      { name: "project", label: "Project Description", type: "textarea", placeholder: "Describe the project...", required: true },
      { name: "period", label: "Project Period", type: "text", placeholder: "e.g., 12 months" },
    ],
    systemPrompt: `You are a grant writing financial specialist. Create a grant budget narrative including: personnel costs with justification, fringe benefits calculation, travel, equipment, supplies, contractual services, indirect costs, and other categories. For each line item provide: amount, calculation methodology, and narrative justification explaining why the expense is necessary and reasonable. Include a summary budget table. Output in markdown.`,
    maxTokens: 5000,
  },
  {
    id: "fin-subscription-analyzer",
    title: "Subscription & SaaS Analyzer",
    category: "Financial Operations",
    description: "Audit SaaS subscriptions, identify waste, and optimize tool stack spending.",
    icon: Cog,
    fields: [
      { name: "subscriptions", label: "Current Subscriptions", type: "textarea", placeholder: "List your SaaS tools and monthly costs...", required: true },
      { name: "teamSize", label: "Team Size", type: "text", placeholder: "e.g., 5" },
    ],
    systemPrompt: `You are a SaaS optimization consultant. Audit the subscription list and provide: total monthly/annual spend, per-employee cost, usage classification (critical, important, nice-to-have, unused), overlap analysis, consolidation opportunities, free alternative recommendations, negotiation tips for renewals, annual savings potential, and prioritized action plan. Output in markdown with tables.`,
    maxTokens: 4000,
  },
  {
    ...
    category: "Financial Operations",
    description: "Build a detailed ROI analysis for projects, investments, or service offerings.",
    icon: TrendingUp,
    fields: [
      { name: "investment", label: "Investment / Project", type: "textarea", placeholder: "Describe the investment...", required: true },
      { name: "cost", label: "Total Cost", type: "text", placeholder: "e.g., $25,000" },
      { name: "timeframe", label: "Timeframe", type: "text", placeholder: "e.g., 12 months" },
    ],
    systemPrompt: `You are a financial analyst. Build a comprehensive ROI analysis including: investment breakdown, benefit quantification (direct and indirect), NPV calculation, payback period, IRR estimate, sensitivity analysis (best/worst/expected), risk factors, assumptions log, and recommendation. Include a month-by-month projection table. Output in markdown.`,
    maxTokens: 5000,
  },
  {
    id: "fin-payroll-planning",
    title: "Payroll & Compensation Planner",
    category: "Financial Operations",
    description: "Plan compensation structures with benefits, taxes, and total cost analysis.",
    icon: Users,
    fields: [
      { name: "roles", label: "Roles & Salaries", type: "textarea", placeholder: "List roles and target salaries...", required: true },
      { name: "state", label: "State", type: "text", placeholder: "e.g., Ohio" },
      { name: "benefits", label: "Benefits to Include", type: "text", placeholder: "e.g., Health, 401k, PTO" },
    ],
    systemPrompt: `You are an HR compensation analyst. Create a payroll plan including: base salary per role, benefits cost breakdown (health insurance, retirement, PTO value), employer tax obligations (FICA, FUTA, SUTA, workers comp), total cost per employee, annual payroll budget, hiring timeline impact, market rate comparison, and equity/bonus structure recommendations. Output in markdown with tables.`,
    maxTokens: 5000,
  },
  {
    id: "fin-break-even-analysis",
    title: "Break-Even Analysis",
    category: "Financial Operations",
    description: "Calculate break-even points for products, services, or business ventures.",
    icon: Target,
    fields: [
      { name: "fixedCosts", label: "Fixed Costs (monthly)", type: "text", placeholder: "e.g., $5,000", required: true },
      { name: "variableCost", label: "Variable Cost per Unit", type: "text", placeholder: "e.g., $15" },
      { name: "pricePerUnit", label: "Price per Unit", type: "text", placeholder: "e.g., $50", required: true },
    ],
    systemPrompt: `You are a financial analyst. Perform a break-even analysis including: break-even point in units and revenue, contribution margin calculation, margin of safety, break-even chart description, sensitivity to price/cost changes, scenarios at different volume levels, time-to-breakeven estimate, and strategic recommendations to lower the break-even point. Output in markdown.`,
    maxTokens: 4000,
  },
  {
    id: "fin-quarterly-report",
    title: "Quarterly Business Report",
    category: "Financial Operations",
    description: "Generate a professional quarterly business report template with financial and operational sections.",
    icon: FileText,
    fields: [
      { name: "quarter", label: "Quarter", type: "text", placeholder: "e.g., Q1 2026", required: true },
      { name: "highlights", label: "Key Highlights", type: "textarea", placeholder: "Major achievements, challenges, numbers..." },
      { name: "metrics", label: "Key Metrics", type: "textarea", placeholder: "Revenue, clients served, growth rate..." },
    ],
