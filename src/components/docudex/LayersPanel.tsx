    ...
    <div className="w-60 border-l border-border bg-card flex flex-col">
      ...
      {selectedElementIds.length === 1 && (
        <div className="px-3 py-2 border-t border-border flex gap-1 justify-center">
          <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => activePageId && reorderElement(activePageId, selectedElementIds[0], "top")} aria-label="Action" title="Bring to front">
            <ChevronsUp className="w-3.5 h-3.5" />
          </Button>
          <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => activePageId && reorderElement(activePageId, selectedElementIds[0], "up")} aria-label="Action" title="Move up">
            <ChevronUp className="w-3.5 h-3.5" />
          </Button>
          <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => activePageId && reorderElement(activePageId, selectedElementIds[0], "down")} aria-label="Action" title="Move down">
            <ChevronDown className="w-3.5 h-3.5" />
          </Button>
          <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => activePageId && reorderElement(activePageId, selectedElementIds[0], "bottom")} aria-label="Action" title="Send to back">
