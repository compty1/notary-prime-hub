...
Deno.serve(async (req) => {
  ...
  // Audit Item 50: Rate limiting (10 req/min for lead discovery)
  ...
  try {
    ...
    const { data: roleData } = await adminCheck.from("user_roles").select("role").eq("user_id", user.id).eq("role", "admin").single();
    if (!roleData) {
